...
public class Schema implements Serializable {
	...
	public static final Schema SCHEMA = new Schema().setNamespace(Namespaces.SYSTEM)
	        ...
	        .setTitle("Schema")
	        .setName("Schema")
	        .setProperties(Map.ofEntries(entry(NAMESPACE_STRING, Schema.of(NAMESPACE_STRING, STRING)),
	                entry("name", Schema.of("name", STRING)), entry(VERSION_STRING, Schema.of(VERSION_STRING, INTEGER)),
	                ...
	                entry("not", Schema.ofRef(SHEMA_ROOT_PATH)), entry("title", Schema.of("title", STRING)),
	                entry("description", Schema.of("description", STRING)), entry("id", Schema.of("id", STRING)),
	                entry("examples",
	                        Schema.ofArray("examples",
	                                Schema.of("example", INTEGER, LONG, FLOAT, DOUBLE, STRING, OBJECT, ARRAY, BOOLEAN,
	                                        NULL))),
	                entry("defaultValue",
	                        Schema.of("defaultValue", INTEGER, LONG, FLOAT, DOUBLE, STRING, OBJECT, ARRAY, BOOLEAN,
	                                NULL)),
	                entry("comment", Schema.of("comment", STRING)),
	                entry("enums", Schema.ofArray("enums", Schema.of("enum", STRING))),
	                entry("constant",
	                        Schema.of("constant", INTEGER, LONG, FLOAT, DOUBLE, STRING, OBJECT, ARRAY, BOOLEAN, NULL)),

	                entry("pattern", Schema.of("pattern", STRING)), entry("format", Schema.of("format", STRING)
	                ...
	                entry("properties", Schema.of("properties", OBJECT)
	                        .setAdditionalProperties(
	                                new AdditionalPropertiesType().setSchemaValue(Schema.ofRef(SHEMA_ROOT_PATH)))),
	                entry("additionalProperties", Schema.of("additionalProperties", BOOLEAN, OBJECT)
	                ...
	                entry("maxProperties", Schema.of("maxProperties", INTEGER)), entry("patternProperties",
	                        Schema.of("patternProperties", OBJECT)
	                ...
	                entry("contains", Schema.ofRef(SHEMA_ROOT_PATH)), entry("minItems", Schema.of("minItems", INTEGER)),
	                entry("maxItems", Schema.of("maxItems", INTEGER)),
	                entry("uniqueItems", Schema.of("uniqueItems", BOOLEAN)),

	                entry("definitions", Schema.of("definitions", OBJECT)
	                        .setAdditionalProperties(
	                                new AdditionalPropertiesType().setSchemaValue(Schema.ofRef(SHEMA_ROOT_PATH)))),
	...
	public static Schema of(String id, SchemaType... type) {
		return new Schema().setType(Type.of(type))
		        .setName(id)
		        .setTitle(id);
	}
