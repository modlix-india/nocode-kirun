...
public class RandomFloat extends AbstractReactiveFunction {
	...
	public FunctionSignature getSignature() {
		return new FunctionSignature().setName("RandomFloat")
		        .setNamespace(MATH)
		        .setParameters(Map.of(MIN_VALUE, new Parameter().setParameterName(MIN_VALUE)
		                .setSchema(Schema.ofFloat(MIN_VALUE)
		                        .setDefaultValue(new JsonPrimitive(0.0f))),
		                MAX_VALUE, new Parameter().setParameterName(MAX_VALUE)
		                        .setSchema(Schema.ofFloat(MAX_VALUE)
		                                .setDefaultValue(new JsonPrimitive(Float.MAX_VALUE)))))
	...
	protected Mono<FunctionOutput> internalExecute(ReactiveFunctionExecutionParameters context) {
		...
		float minValue = context.getArguments()
		        .get(MIN_VALUE)
		        .getAsFloat();

		float maxValue = context.getArguments()
		        .get(MAX_VALUE)
		        .getAsFloat();
