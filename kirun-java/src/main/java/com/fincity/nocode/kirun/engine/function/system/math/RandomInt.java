...
public class RandomInt extends AbstractFunction {
	...
	public FunctionSignature getSignature() {
		return new FunctionSignature().setName("RandomInt").setNamespace(MATH)
				.setParameters(Map.of(MIN_VALUE,
						new Parameter().setParameterName(MIN_VALUE)
								.setSchema(Schema.ofInteger(MIN_VALUE).setDefaultValue(new JsonPrimitive(0))),
						MAX_VALUE,
						new Parameter().setParameterName(MAX_VALUE).setSchema(
								Schema.ofInteger(MAX_VALUE).setDefaultValue(new JsonPrimitive(Integer.MAX_VALUE)))))
