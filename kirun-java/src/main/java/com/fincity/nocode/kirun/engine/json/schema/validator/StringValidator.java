...
public class StringValidator {
    ...
    public static JsonElement validate(List<Schema> parents, Schema schema, JsonElement element) {
        ...
        if (schema.getMinLength() != null && length < schema.getMinLength()) {
            String actualValue = jp.getAsString();
            String msg = ReactiveSchemaUtil.customMessageProcessor(
                    uiHelper != null ? (String) uiHelper.get("minLength") : null, actualValue);

            throw new SchemaValidationException(path(parents),
                    msg != null ? msg : "Expected a minimum of " + schema.getMinLength() + " characters");
        } else if (schema.getMaxLength() != null && length > schema.getMaxLength()) {
            String actualValue = jp.getAsString();
            String msg = ReactiveSchemaUtil.customMessageProcessor(
                    uiHelper != null ? (String) uiHelper.get("maxLength") : null, actualValue);

            throw new SchemaValidationException(path(parents),
                    msg != null ? msg : "Expected a maximum of " + schema.getMaxLength() + " characters");
        }
    ...
    private static void patternMatcher(List<Schema> parents, Schema schema, JsonElement element, JsonPrimitive jp,
        ...
        if (!matched) {
            String actualValue = jp.getAsString();
            String msg = ReactiveSchemaUtil.customMessageProcessor(
                    schema.getUiHelper() != null ? (String) schema.getUiHelper().get("pattern") : null, actualValue);

            throw new SchemaValidationException(path(parents),
                    msg != null ? msg : element.toString() + " is not matched with the " + message);
        }
    }
