		...
		List<EventResult> out = new KIRuntime(
				((FunctionDefinition) new FunctionDefinition().setNamespace("Test").setName("SingleCall")
						...
						.setSteps(Map.ofEntries(
								Statement.ofEntry(
										new Statement("first").setNamespace(abs.getNamespace()).setName(abs.getName())
	...
	void testHyp() {
		...
		List<EventResult> out = new KIRuntime(
				...
				.execute(new FunctionExecutionParameters()
						.setArguments(Map.of("value1", new JsonPrimitive(3), "value2", new JsonPrimitive(4))))
				.allResults();

		assertEquals(List.of(new EventResult().setName("output").setResult(Map.of("value", new JsonPrimitive(5.0d)))),
				out);
	}
