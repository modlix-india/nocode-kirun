...
class PostPadTest {
	...
	void test() {
		...
		assertEquals(new JsonPrimitive(" THIScompatY IS A NOcoDE plATFNORMhiranhiranhi"),
				postpad.execute(new FunctionExecutionParameters().setArguments(Map.of(PostPad.PARAMETER_STRING_NAME,
	...
	void test2() {
		...
		assertEquals(new JsonPrimitive(" THIScompatY IS A NOcoDE plATFNORM h  h  h  h"),
				prepad.execute(new FunctionExecutionParameters().setArguments(Map.of(PostPad.PARAMETER_STRING_NAME,
						new JsonPrimitive(s1), PostPad.PARAMETER_POSTPAD_STRING_NAME, new JsonPrimitive(s2),
