...
public class RegionMatches extends AbstractFunction {
	...
	protected static final Parameter PARAMETER_OTHER_STRING = new Parameter()
			.setParameterName(PARAMETER_OTHER_STRING_NAME).setSchema(Schema.ofString(PARAMETER_OTHER_STRING_NAME));

	protected static final Parameter PARAMETER_FIRST_OFFSET = new Parameter()
			.setParameterName(PARAMETER_FIRST_OFFSET_NAME).setSchema(Schema.ofInteger(PARAMETER_FIRST_OFFSET_NAME));

	protected static final Parameter PARAMETER_SECOND_OFFSET = new Parameter()
			.setParameterName(PARAMETER_SECOND_OFFSET_NAME).setSchema(Schema.ofInteger(PARAMETER_SECOND_OFFSET_NAME));
	...
	protected FunctionOutput internalExecute(FunctionExecutionParameters context) {
		...
		Boolean ignoreCase = context.getArguments().get(PARAMETER_BOOLEAN_NAME).getAsBoolean();
		Integer toffSet = context.getArguments().get(PARAMETER_FIRST_OFFSET_NAME).getAsInt();
		String otherString = context.getArguments().get(PARAMETER_OTHER_STRING_NAME).getAsString();
		Integer oOffSet = context.getArguments().get(PARAMETER_SECOND_OFFSET_NAME).getAsInt();
		Integer length = context.getArguments().get(PARAMETER_INTEGER_NAME).getAsInt();

		Boolean matches = inputString.regionMatches(ignoreCase, toffSet, otherString, oOffSet, length);

		return new FunctionOutput(List.of(EventResult.outputOf(Map.of(EVENT_RESULT_NAME, new JsonPrimitive(matches)))));
	}
}
