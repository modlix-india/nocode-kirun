...
const TOKEN_FUNCTION: any = {
    ...
    d: (str: string, pattern: string, date: Date) => {
        ...
        if (pattern.startsWith('dddd')) {
            return { str: str + DAYS_OF_WEEK[dayOfTheWeek], pattern: pattern.slice(4) };
        } else if (pattern.startsWith('ddd')) {
            return { str: str + DAYS_OF_WEEK[dayOfTheWeek].slice(0, 3), pattern: pattern.slice(3) };
        } else if (pattern.startsWith('dd')) {
            return { str: str + DAYS_OF_WEEK[dayOfTheWeek].slice(0, 2), pattern: pattern.slice(2) };
        } else if (pattern.startsWith('dth')) {
            return { str: str + numberToOrdinal(dayOfTheWeek + 1), pattern: pattern.slice(3) };
        } else if (pattern.startsWith('dTH')) {
            return {
    ...
    N: (str: string, pattern: string, date: Date) => {
        if (pattern.startsWith('NNNN')) {
            str += date.getFullYear() < 0 ? 'Before Common Era' : 'Common Era';
            return { str, pattern: pattern.slice(4) };
        }
...
export function dateToTimeStampObject(date: Date): TimeStampObject {
    return {
...
export function dateFromFormattedString(dateString: string, formatString: string): Date {
