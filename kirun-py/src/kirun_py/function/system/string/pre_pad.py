...
class PrePad(AbstractFunction):

    PARAMETER_STRING_NAME: str = 'string'
    PARAMETER_PREPAD_STRING_NAME: str = 'prepadString'
    ...
    def __init__(self) -> None:
        ...
        param_prepad = Parameter(
            self.PARAMETER_PREPAD_STRING_NAME,
            Schema.of_string(self.PARAMETER_PREPAD_STRING_NAME),
        )
    ...
    async def internal_execute(
        ...
        prepad_string: str = context.get_arguments().get(self.PARAMETER_PREPAD_STRING_NAME)
        length: int = context.get_arguments().get(self.PARAMETER_LENGTH_NAME)

        output_string = ''
        pad_len = len(prepad_string)
        current_len = pad_len

        while current_len <= length:
            output_string += prepad_string
            current_len += pad_len

        if len(output_string) < length:
            output_string += prepad_string[:length - len(output_string)]
