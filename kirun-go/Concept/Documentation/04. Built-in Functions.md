...
1. **Variable Creation and Manipulation**:
   ...
   ```kirun
   create: System.Context.Create(name = "counter", schema = INTEGER)
   increment: System.Math.Add(value = Context.counter, value = 1)
   update: System.Context.Set(name = "counter", value = Steps.increment.output.value)
   ```

2. **Conditional Logic**:

   ```kirun
   check: System.If(condition = Context.counter > 10)
       true
           reset: System.Context.Set(name = "counter", value = 0)
       false
           continue: System.Math.Add(value = Context.counter, value = 1)
   ```

3. **Array Processing**:

   ```kirun
   process: System.Loop.ForEachLoop(source = Arguments.items)
       iteration
           transform: System.String.UpperCase(string = Steps.process.iteration.each)
           collect: System.Array.InsertLast(source = Context.results, element = Steps.transform.output.result)
   ```

4. **Error Handling**:
   ```kirun
   parse: System.JSON.JSONParse(source = Arguments.jsonString)
       error
           log: System.Print(stream = "error", values = Steps.parse.error.errormessage)
       output
           process: System.Object.ObjectKeys(source = Steps.parse.output.value)
   ```

### Event-Driven Flow

All built-in functions participate in KIRun's event-driven execution model:

- Functions generate events when they complete
- Events carry data through event parameters
- Subsequent steps depend on these events
- The indentation in text format becomes dependency tracking in JSON

This comprehensive set of built-in functions provides the foundation for building complex business logic and data processing workflows in KIRun applications.
